...
\COPY taxref_protection_articles (cd_protection, article, intitule, arrete, url_inpn, cd_doc, url, date_arrete, type_protection) FROM  '/tmp/taxhub/PROTECTION_ESPECES_TYPES_11.csv' WITH  CSV HEADER;
...
\COPY import_protection_especes FROM  '/tmp/taxhub/PROTECTION_ESPECES_11.csv' WITH  CSV HEADER;

---import liste rouge--
TRUNCATE TABLE taxonomie.taxref_liste_rouge_fr;
\COPY taxonomie.taxref_liste_rouge_fr (ordre_statut,vide,cd_nom,cd_ref,nomcite,nom_scientifique,auteur,nom_vernaculaire,nom_commun,  rang,famille,endemisme,population,commentaire,id_categorie_france,criteres_france,liste_rouge,fiche_espece,tendance,     liste_rouge_source,annee_publication,categorie_lr_europe,categorie_lr_mondiale) FROM  '/tmp/taxhub/LR_FRANCE.csv' WITH  CSV HEADER DELIMITER E'\;'  encoding 'UTF-8';
